...
const webappUrl = "http://localhost:3000/index.html"; // location of webapp
...
describe("Example-Webapp", function () {
  before(() => require("../app"))
  describe("register new user", function () {
    ...
    afterEach(
      async () => await driver.quit() // quits the selenium webdriver after each test.
    );
    ...
    it("sends email with link to example.com website", async () => {
      ...
      await driver
        ...
        .sendKeys("ExampleUser"); // find element "username" and enters the tet "ExampleUser"
      await driver
        .findElement(webdriver.By.id("email"))
        .sendKeys(signupEmailAddress); // find element "email" and enters the value for the variable signupEmailAddress (ie user1@mailsac.com)
      await driver.findElement(webdriver.By.id("submitUserCreation")).click(); // click the submit button on the form
      await driver.wait(webdriver.until.elementLocated(webdriver.By.id("success")), 5000); // wait for success page to load
      ...
      for (let i = 0; i < 10; i++) {
        ...
        messages = res.body;
        if (res.status == 401) {
          throw new Error("Failed to authenticate to Mailsac API. Did you enter your API in the variable MailsacAPIKey")
        }
